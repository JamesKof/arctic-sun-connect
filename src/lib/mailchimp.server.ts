// Server-only Mailchimp helper. Mirrors confirmed subscribers into an audience.
// Uses the Marketing API directly (Mailchimp is not a Lovable connector).
...
function md5Hex(input: string): string {
  // Minimal MD5 implementation for Mailchimp subscriber-hash (lowercased email).
  // Sourced from public domain reference. Runs in Cloudflare Workers.
  /* eslint-disable */
  function rh(n: number, j: number) {
    const s = "0123456789abcdef";
    let str = "";
    for (let i = 0; i <= j; i++) str += s.charAt((n >> (i * 8 + 4)) & 0x0f) + s.charAt((n >> (i * 8)) & 0x0f);
    return str;
  }
  ...
  return rh(a, 3) + rh(b, 3) + rh(c, 3) + rh(d, 3);
  /* eslint-enable */
}
...
/**
 * Upsert a member into the configured Mailchimp audience.
 * Silently no-ops (returns { skipped: true }) when Mailchimp env vars are not configured,
 * so newsletter signups still work before the customer wires Mailchimp.
 */
export async function upsertMailchimpMember(
  ...
  const prefix = process.env.MAILCHIMP_SERVER_PREFIX || (apiKey ? serverPrefixFromKey(apiKey) : null);
  ...
  const url = `https://${prefix}.api.mailchimp.com/3.0/lists/${audienceId}/members/${hash}`;
  ...
  if (!res.ok) {
    ...
    console.error(`Mailchimp upsert failed [${res.status}]: ${text}`);
    // Don't throw — Mailchimp mirror is a best-effort side effect.
