...
export function Header() {
  return (
    <header className="glass fixed top-0 z-50 w-full border-b border-arctic-deep/5 animate-fade-up">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
        <Link to="/" aria-label="Afro Polar Institute — Home" className="transition-transform duration-300 hover:scale-[1.03]">
          <Logo />
        </Link>
        <div className="hidden items-center gap-8 text-[13px] font-medium uppercase tracking-[0.14em] text-arctic-deep/80 md:flex">
          {nav.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="link-underline transition-colors hover:text-aurora"
